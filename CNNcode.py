...
def training_mask_generation(input_image_filename, input_geojson_filename):
    ...
    for polygon in range(len(polygons['features'])):
        coords = np.array(polygons['features'][polygon]['geometry']['coordinates'])                      
        print(coords[:,0])
...
def training_data_generation(DATA_DIR, img_height_size, img_width_size, perc, buff):
    ...
    #print(DATA_DIR + '\\' + '*.png')
    img_files = glob.glob(DATA_DIR + '\\' + '*.png')
    #print((DATA_DIR + '\\' + '*.png'))
    polygon_files = glob.glob(DATA_DIR + '\\' + '*.geojson')
...
def image_model_predict(input_image_filename, output_filename, img_height_size, img_width_size, fitted_model, write):
    ...
    print("read successfull", img.shape)
...
#import os
#print(os.getcwd())#
c,d=training_data_generation(r'C:\Users\TorriX\Desktop\images', 1024, 1024, 0.1, 0.1
                             )
#e=FCN_VGG16_model(512, 512, transfer_learning = True, l_r = 0.0001)
#f=image_model_predict(r'C:\Users\smegh\OneDrive\Desktop\Images\TestImage.png', 'Output', 512, 512, e, True)
#print(f)
#Mask1 = Image.fromarray(f, 'RGB')
#Mask1.save('my.png')
#Mask1.show()
#print(a)
#print(b)
